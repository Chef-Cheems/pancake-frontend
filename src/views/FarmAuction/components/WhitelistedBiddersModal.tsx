...
import { Modal, Box, Text, Flex, Input, OpenNewIcon, useMatchBreakpoints } from '@pancakeswap/uikit'
...
const WhitelistedBiddersModal: React.FC<WhitelistedBiddersModalProps> = ({ onDismiss }) => {
  ...
  const filteredBiders = bidders.filter(
  ...
  return (
    ...
    >
      ...
      <Box pb="24px" overflow="scroll" maxHeight="245px">
        {filteredBiders.map((bidder) => (
          <AddressRow bidder={bidder} isMobile={isXs || isSm} />
        ))}
      </Box>
    </StyledModal>
  )
